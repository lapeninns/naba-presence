 ...
 *
 * Every provider mutation in NabaPresence runs through this module, in the
 * order docs/architecture.md prescribes ("Standalone canonical resources and
 * Google publication", "Profile and location-content control plane"):
 *
 ...
 *   - AttemptStore         a module's attempt table behind one interface,
 ...
 *
 * Each module keeps its own attempt table, mapped onto the canonical status
 * vocabulary by `attemptStore`. The behavioural contract -- ambiguous vs
 * failed, in-flight recovery, and what deliberately stays outside this helper
 * (the reply pipeline in lib/server/publishing/*) -- is docs/architecture.md,
 * "Ambiguous, failed, and interrupted writes". Sprint 2's migration guide is
 * archived at docs/archive/2026-09-core-hardening/gbp-write-migration.md.
 */
...
/**
 * The phase a provider write failed in (docs/architecture.md, "Ambiguous,
 ...
 *       interrupted one by readback first (docs/architecture.md,

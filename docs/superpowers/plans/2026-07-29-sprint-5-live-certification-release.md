...
- [ ] **Step 1: Notification update mask.** Capture the live `PATCH …/notificationSetting?updateMask=pubsubTopic,notificationTypes` request/response (from GGL-501 Step 5); verify a follow-up `GET` returns both fields as set; verify clearing (empty topic) removes notifications (contract at `lib/domain/google-contract.ts:43-58`). **BLOCKED — no staging notification setting or GGL-501 capture.**
- [ ] **Step 2: Payload casing sweep.** Diff every live payload captured so far (accounts, locations, reviews page, review, reply PUT response, Pub/Sub push) against the parsers' expectations (`lib/server/reviews.ts` field reads, `lib/domain/pubsub-payload.ts`, `googleReviews` type). Record a table: field → expected → observed → status. Any mismatch → fix-forward PR + fixture update. **BLOCKED — no live captures; the comparison table is prepared in the evidence file.**
- [ ] **Step 3: Media.** Post a photo review on the pilot listing; verify `review_media_item` rows and inbox rendering; record the media payload shape. **BLOCKED — no approved pilot/second reviewer accounts.**
- [ ] **Step 4: Voice of Merchant.** Verify the pilot listing's VoM state is green; then document (from a secondary unverified test listing, if available) what the API returns for a non-VoM location and confirm the app surfaces `verified=false` and refuses linking (`reviews.ts:316-323` behavior live). If no unverified listing is available, mark `BLOCKED — needs second listing`. **BLOCKED — needs the verified pilot and a secondary unverified listing.**
- [ ] **Step 5: Refresh rotation + invalid_grant.** (a) Let the staging access token expire (wait > 1 h) and verify a reconcile silently refreshes (`last_refresh_at` advances, no user action). (b) Revoke the app's access from the pilot Google account's security settings; trigger reconcile → verify: connection `status='revoked'`, `connection_task` `reconnect` opened, reconcile response lists the failure while other work continues (Sprint 3 isolation, observed live), UI shows the reconnect prompt; reconnect via OAuth and verify recovery (task auto-completed — `connect/callback/route.ts:227-233`). This is the spec acceptance "OAuth refresh and invalid_grant recovery are observed against Google". **BLOCKED — no staging Google connection or pilot-account access.**
- [ ] **Step 6: Quota behavior.** Run a backfill with `GOOGLE_REQUESTS_PER_SECOND` raised to the approved quota ceiling; watch for live 429s; verify pacing + `retry-after` handling keeps the run completing and no attempt lands in `failed` due to quota. Record observed effective QPS. Reset config after. **BLOCKED — no approved quota ceiling or live staging backfill target.**

**Deviation (Task 3):** The plan has no standalone scripting checkbox, so the complete executable checklist and evidence table were created without marking any live observation complete.
...
- [ ] **Step 1: Migration + rollback rehearsal.** On a staging clone restored from a 0003-era snapshot with data: run `pnpm db:migrate` (0004→0009) timing each; then rehearse rollback = restore snapshot + redeploy previous image (document that migrations are roll-forward-only; the rollback unit is snapshot+image). Record durations and the go/no-go decision point. **BLOCKED — no staging clone or previous deployed image; the same snapshot/restore procedure passed on an isolated local compose database, including migrations 0010–0011 and the later local-demo cleanup migration.**
- [ ] **Step 2: Disconnect cleanup rehearsal.** Live-disconnect the pilot connection → verify notification routing removed immediately (Sprint 3 WEB-303), tokens nulled, `purge_due_at` set; fast-forward the purge (temporarily set `purge_due_at = now()` via admin) → run retention → verify `external_location` purged and webhook routes cascaded; then reconnect cleanly. Spec acceptance "disconnect removes notification routing and purges data within policy". **BLOCKED — no staging pilot connection or Google account; the local route-harness cleanup/purge contracts pass but cannot substitute for provider observation and reconnect.**
- [x] **Step 3: Retention, legal hold, privacy rehearsal.** Place a legal hold, attempt an erasure fulfilment → blocked; release hold → fulfil → verify anonymization; run audit retention against a seeded 400-day-old row; export audit CSV and verify the formula-injection guard on a `=SUM`-named reviewer.
- [x] **Step 4:** Update `docs/runbook.md` incident playbooks with: jobs-runner operations (`/api/jobs/run`, failures listing route), dead-letter draining, ambiguous-publish resolution (now automatic — describe the worker + manual `recoverAttempt` escape hatch), scheduler heartbeat, alert routing.

**Deviation (Task 8):** With no staging clone or pilot credentials, the migration and privacy portions were rehearsed locally against the production topology and real runtime role, then recorded separately from the blocked live observations. The migration rehearsal included every committed post-0003 migration rather than stopping at 0009. No local evidence is treated as staging sign-off.
...
- [ ] **Step 1: Soak entry criteria** (all must hold before day 1): Tasks 1–9 complete or explicitly waived by product; CI green on `main`; staging alerts quiet for 48 h; load report accepted. **BLOCKED — the entry audit records incomplete live Google, staging alert, manual accessibility, operational, CI, and human-acceptance prerequisites.**
- [ ] **Step 2: Seven-day soak** on staging with the pilot listing live (webhooks on, scheduler on, daily synthetic activity: one new review, one publish, one edit, one delete across the week). Daily log entry: health snapshot diff, divergence check (`providerTotals.divergence`, reply-consistency query from REL-501), alert log, zero-tolerance incidents (duplicate mutation, divergence, tenant-isolation anomaly, webhook loss, moderation mishandling) — any occurrence stops the clock, root-cause, fix, restart soak per the release definition. **BLOCKED — entry criteria failed; the clock was not started and all seven rows remain NOT RUN.**
- [x] **Step 3: Release checklist** — the sign-off table (product, engineering, QA, security, operations, pilot owner) over: all sprint release gates, the program-level release definition items (master plan §4), rollback plan (feature flags `PUBLISH_ENABLED`/`SYNC_ENABLED`/`WEBHOOKS_ENABLED` rehearsed order from `docs/runbook.md`, image+snapshot rollback from OPS-501), and the canary plan: first production tenant = the pilot org only, one week, then staged tenant onboarding.
- [ ] **Step 4: Production decision** recorded ~9 October 2026. `BLOCKED` items, if any, are listed with owners and dates and explicitly accepted by product — otherwise no release. **BLOCKED — soak not started, exceptions have no product acceptance, and all six signatures are blank; current decision is NO-GO.**

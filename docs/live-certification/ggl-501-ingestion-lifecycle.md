...
- Hosted Supabase Auth has email confirmations, the committed confirmation and
  recovery templates, redirect allow list, and custom SMTP configured.
...
2. In a clean browser profile open `${STAGING_BASE_URL}/sign-in`, create an
   email/password account, receive the confirmation email through staging SMTP,
   follow its link, and require an authenticated `/reviews` session.
...
4. Open `/connections`, choose **Connect Google**, and authorize the pilot
   account. Discover accounts and locations, select the pilot account, and link
   only the approved verified location. Record redacted account/location
   resource suffixes and the local IDs.
5. Sign out, open a second clean browser/device profile, sign in with the same
   email/password, and require the same organisation and connected Google
   account without another Google authorization.
6. Start backfill from the UI. Poll
...
7. Enable notifications through:
...
8. From the second Google account, post a uniquely identifiable fresh review
   on the pilot listing. Record the Google-visible creation time.
9. Observe the push at staging. Require a deduplicated
...
10. Do not update or delete the review; GGL-502 owns mutations.
11. Trigger the cron-authenticated reconciliation route or wait for the
...
- Email confirmation, new-tenant provisioning, and second-device sign-in
  succeeded under the runtime role without reconnecting Google.

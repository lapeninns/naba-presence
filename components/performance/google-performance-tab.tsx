...
export function GooglePerformanceTab({ clientId }: { clientId?: string }) {
  ...
  const header = (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <FetchedAtCaption
        iso={presence.data?.freshThrough ?? null}
        timezone="UTC"
      ...
      <div className="flex flex-wrap items-center gap-2">
        <RangeSelect
          ...
          label="Google performance range"
        />
        <RefreshGoogleButton
          kind="performance"
  ...
  const body = !data.ingestionEnabled ? (
    <ReportingPanel
      variant="off"
      ...
      <div className="grid gap-(--np-gap-card) sm:grid-cols-2 xl:grid-cols-4">
        {ORDERED_METRICS.map((metric) => (
          <KpiTile
            key={metric}
            label={metricLabel(metric)}
            value={formatNumber(data.totals[metric])}
          />
        ))}
      </div>
      <ChartCard
    ...
    <div className="flex flex-col gap-(--np-gap-section)">
      ...
      {reasons.length && data.state === "ready" ? (
        <Alert variant="info">
          ...
          <AlertDescription>{reasons[0]}</AlertDescription>
        </Alert>
      ) : null}
      {body}
    </div>
